		         ...
		         public void prepare(MimeMessage mimeMessage) throws Exception {
		            ...
		            message.setFrom("admin@wikicrimes.org"); 
						...
						if (locale.equals("pt_BR") || locale.equals("pt")){
							message.setSubject("Altera��es no registro de seu crime - WikiCrimes.org");
						 text = VelocityEngineUtils.mergeTemplateIntoString(
				...
				public void prepare(MimeMessage mimeMessage) throws Exception {
					...
					if (locale.equals("pt_BR") || locale.equals("pt")){
						...
						if(relato.getSubTipoRelato().equalsIgnoreCase("6"))
							model.put("subTipoRelato", "Viol�ncia contra Crian�a");
					if(confirmacao.getUsuario().getPerfil().getIdPerfil()== Perfil.CONVIDADO){
						text = VelocityEngineUtils.mergeTemplateIntoString(
		...
		final Crime crime = confirmacao.getCrime();
			...
			try {
				Thread t = new Thread() {
					public void run() {
						mailSender.send(preparator);
						System.out.println("[" + new Date() + "] Email de Notifica��o de confirmacao de Crime por " + confirmacao.getUsuario().getEmail() + " para " + usuario.getEmail());
					}
				};
				t.start();
				System.out.print("Enviando email...");
		...
		final Relato relato = confirmacao.getRelato();
			...
			try {
				Thread t = new Thread() {
					public void run() {
						mailSender.send(preparator);
						System.out.println("[" + new Date() + "] Email de Notifica��o de confirmacao de Relato por " + confirmacao.getUsuario().getEmail() + " para " + usuario.getEmail());
					}
				};
				t.start();
				System.out.print("Enviando email...");
		...
		.getFactory(FactoryFinder.APPLICATION_FACTORY);
			...
			try {
				Thread t = new Thread() {
					public void run() {
						mailSender.send(preparator);
						System.out.println("[" + new Date() + "] Email de Notifica��o de coment�rio de " + comentario.getUsuario().getEmail() + " enviado para " + usuario.getEmail());
					}
				};
				t.start();
				System.out.print("Enviando email...");
