...
Clusterer.defaultGridSize = 8;
Clusterer.defaultMinMarkersPerCluster = 8;
Clusterer.defaultMaxLinesPerInfoBox = 6;
Clusterer.defaultIcon = new GIcon();
Clusterer.defaultIcon.image = "http://econym.googlepages.com/blue_large.png";
Clusterer.defaultIcon.shadow = "http://econym.googlepages.com/shadow_large.png";
	...
	if (visibleMarkers.length > clusterer.maxVisibleMarkers) {
		...
		for (i = 0; i < clusterer.clusters.length; ++i) {
			...
			if (cluster != null && cluster.marker == null) {
				...
				GEvent.addListener(marker, "mouseover", Clusterer.MakeCaller(Clusterer.OverUp, cluster));
			}
		}
	}
	for (i = 0; i < visibleMarkers.length; ++i) {
	...
	for (var i = 0; i < cluster.markers.length; ++i) {
		...
		if (marker != null) {
			...
			if (n == clusterer.maxLinesPerInfoBox - 1 && cluster.markerCount > clusterer.maxLinesPerInfoBox) {
				html += "<tr><td colspan=\"2\">...e outros " + (cluster.markerCount - n) + " crimes</td></tr>";
				break;
			}
		}
	}
...
Clusterer.OverUp = function (cluster) {
	var clusterer = cluster.clusterer;

	mostraTooltip(cluster.markerCount);
