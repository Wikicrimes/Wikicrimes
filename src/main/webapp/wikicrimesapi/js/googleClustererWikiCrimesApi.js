...
function MarkerClusterer(map, opt_markers, opt_opts) {
  ...
  for (i = 1; i <= sizes.length; ++i) {
    styles_.push({
      'url': urlWikiCrimes + 'images/baloes/clusterIcon' + i + '.png',
      'height': sizesH[i - 1],
      'width': sizes[i - 1],
      'opt_textColor': 'white'
    });
...
ClusterMarker_.prototype.initialize = function (map) {
 ...
 for(var i=0; i < vetorMarcador.length; i++) {
  ...
  GEvent.addDomListener(div, "mouseover", function () {
	  ...
	  tooltip.innerHTML = '<div style="margin: 0; padding: 2px 5px 2px 5px; font: normal normal normal 8pt Arial; background-color: #F0F8FF; border: 1px Gray solid;"><strong><center><nobr>' + qtdMarker + ' ' + mensagens['agrupados'] + '</center></strong><br /><img src="http://chart.apis.google.com/chart?cht=p&chd=t:' + ((TRoubo/qtdMarker)*100).toFixed(2) + ',' + ((TFurto/qtdMarker)*100).toFixed(2) + ',' + ((TOutro/qtdMarker)*100).toFixed(2) + '&chs=200x70&chdl='+mensagens['tipo.crime.roubo']+' (' + TRoubo + ')|'+mensagens['tipo.crime.furto']+' (' + TFurto + ')|'+mensagens['tipo.crime.outros']+' (' + TOutro + ')&chco=A52A2A|0066FF|FF9933&chf=bg,s,65432100"/></div>';
